...
function getEvent(program_id, orgunit_id, cycle_start, cycle_end){
    ...
    // console.log("Full analytics streng: ", ('../api/analytics/events/query/' + program_id + '?dimension=ou:' + orgunit_id + '&' + data_elements_string + 'startDate=' + cycle_start + '&endDate=' + cycle_end))
    $.ajax({
        ...
        success: function (json_first_half) {
            // console.log("json_first_half", json_first_half)
            all_data = json_first_half
        }
    })

    // console.log("her", all_data)
    var data_elements_string = ''
    for(var i = (Math.floor(data_element_ids.length/2) + 1); i < data_element_ids.length; i++){
        data_elements_string +=  'dimension=' + data_element_ids[i] + '&'
    }

    // console.log("Full analytics streng:2 ",  (base_url + '/api/analytics/events/query/' + program_id + '?dimension=ou:' + orgunit_id + '&' + data_elements_string + 'startDate=' + cycle_start + '&endDate=' + cycle_end + '&_' + new Date().getTime()))
    return $.get( base_url + '/api/analytics/events/query/' + program_id + '?dimension=ou:' + orgunit_id + '&' + data_elements_string + 'startDate=' + cycle_start + '&endDate=' + cycle_end + '&_' + new Date().getTime(), function( json_second_half ) {
        // console.log("json_second_half", json_second_half)
        // console.log("height: " , all_data.height, "start_date: ", cycle_start, "cycle_end:", cycle_end, all_data)
...
function getRelevantData(kind_of_data, filter, fields, callback){
    // console.log("kaller: ", (base_url + '/api/' + kind_of_data + '.jsonp?' + filter + 'paging=false&fields=' + fields))
    return $.ajax({
...
function getCyclePeriods(org_unit_zone){
    // console.log("orgunit_zone: ", org_unit_zone)
...
function generateFromEventById(event_uid){
    ...
    for(var i = 0; i < order_periods.length; i++){
        ...
        // console.log("cycle_end: ", cycle_end)
    ...
    return $.when.apply($, promises).then(function() {
        ...
        return event_promise[0].then(function(){
            ...
            setSections('ARV', 0).then(function(){
                createSectionObject(0)
                createFormObject(0).then(function(){
                    createCycleObject(0)
                })
            })
        })
...
function generateFromEvents(){
    ...
    return $.when.apply($, promises).then(function() {
        for(var i = 0; i < promises.length; i++){
            if (all_api_events[i] && all_api_events[i].height > 0){
                createCommodityObject(undefined, i)
                setSections('ARV', i).then(function(){
                    createSectionObject(i)
                    createFormObject(i).then(function(){
                        createCycleObject(i)
                    })
                })
            }
...
function setSections(program_name, event_number){
    return $.ajax({
        ...
        success: function (json) {
            ...
            // console.log("all_api_sections", all_api_sections, "notApplicableDict",notApplicableDict)
...
function createCommodityObject(event_uid, event_number){
    ...
    for(var i = DATA_ELEMENT_VALUES_START; i < event.headers.length; i++){
        ...
        if (dataElementObj.name == 'applicable'){
            ...
            // console.log("data_elem_name", dataElementObj.name)
            dataElementObj.value = parseFloat(event.rows[0][i])
            // console.log("data_elem_value", dataElementObj.value)
            dataElementObj.type = event.headers[i].valueType
            // console.log("data_elem_type", dataElementObj.type, "\n")
        ...
        // console.log("\nmekka meg litt commodity da: ", commodities[commodity_counter], "\n")
    }
}
...
function createCycleObject(event_number){
    cycles[event_number] = {};
    cycles[event_number].id = event_number;
    cycles[event_number].forms =  [];
    cycles[event_number].forms = forms[event_number];
}
...
// CREATE_AND_UPDATE eLLER UPDATE
...
// function getFormNumberByFormId(form_id){
...
function updateEventWithBlanks(orgunit_id, event_id){
    ...
    var jsn = createPostEvent(orgunit_id, createDataValuesArray(all_ids, all_values));
...
function magnus_funksjon2(){
    return server_interface.setForm('BQfsyXkVIbw').then(function(){ // første sykel for kawalaa
        // setTimeout(function(){
        form = forms[0]
        console.log("her er formet ditt,magnus li",form)
        // console.log("her er formet ditt,magnus li", JSON.stringify(form.sections[0].commodities[0].dataElements), form)
        // }, 3000);
    })
}
